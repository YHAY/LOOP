...
def delete():
    ...
    if (count > -1):
        ...
        for i in files:  # files : [record.py, record0.py...]
            ...
            os.remove(i)
            print
            "file name [" + i + "]"  # show the remain lists : empty= success
        count = 0
    lcds()
...
def save():
    ...
    if count == 1:
        ...
        out_stream = p.open(format=p.get_format_from_width(wf2.getsampwidth()), channels=wf2.getnchannels(),
                            rate=wf2.getframerate(), output=True)
